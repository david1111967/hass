...
function le_renew() {
    ...
    dehydrated --cron --algo "${ALGO}" --hook ./hooks.sh --out "${CERT_DIR}" --config "${WORK_DIR}/config" || true
    LE_UPDATE="$(date +%s)"
}
...
while true; do
    ...
    if [[ -z ${ipv4} || ${ipv4} == *.* ]]; then
        if answer="$(curl -s "http://${USER}:${TOKEN}@www.dinadns.es/update?hostname=${DOMAINS}&myip=${ipv4}")" && [ "${answer}" != 'good' ]; then
            bashio::log.info "${answer}"
        else
            bashio::log.warning "${answer}"
        fi
